...
### For Existing Users
Run `npx inner-dialogue@latest update --path <your-folder>` to receive the hook and its registration automatically — the script is copied to `.therapy/hooks/safety-net.js` and appended to `.claude/settings.json` (backup taken first). Your `profile.md`, `sessions/`, and `CLAUDE.md` remain untouched as always.
